...
- **[FreeRTOS-Kernel PR #1492](https://github.com/FreeRTOS/FreeRTOS-Kernel/pull/1492)**（当前待合并，获 NXP 工程师 review approve）— 定位并修复 IAR TrustZone non-secure 端口 `portasm.s` 的汇编缺陷：误粘的安装包 URL 落在 `/* */` 注释之外，汇编器将其当作 `ldmia` 的操作数，导致该端口在**非 MPU 配置**下构建失败。实测受影响文件为 **7 个**（上游 issue 仅报告 1 个），并用 `git log -S` 溯源至引入提交。
- **[pymodbus PR #3023](https://github.com/pymodbus-dev/pymodbus/pull/3023)**（**已合并**，squash `55ec55e`，2026-09-15）— 修复模拟器数据存储模块的行为缺陷（issue #3022），+6 / −6，上游 `test/simulator/` 回归套件 277 项全部通过。
